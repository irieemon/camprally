 ...
 *   src/data/prices.json   only what the site renders, imported at build time
 *
...
// Emit the render file: only priced entries, only fields the site uses.
const render = {};
for (const [asin, e] of Object.entries(cache.entries)) {
  ...
  render[asin] = {
    ...
    asOf: e.priceCheckedAt,
  };
}
const payload = {
  generatedAt: nowIso,
  note: "Generated by scripts/refresh-prices.mjs. Do not edit by hand.",
  prices: render,
};
mkdirSync(`${ROOT}src/data`, { recursive: true });
writeFileSync(`${ROOT}src/data/prices.json`, JSON.stringify(payload, null, 2) + "\n");

console.log(
  `\npriced ${priced} · failed ${failed} · ` +
  `src/data/prices.json now carries ${Object.keys(render).length} live price(s)`,
);
